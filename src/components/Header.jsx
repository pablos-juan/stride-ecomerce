  ...
  return (
    <header className='flex flex-col md:flex-row justify-center md:items-center p-1 md:p-4 m-2 gap-2'>
      ...
      <div className='flex gap-2'>
        ...
        <button onClick={handleClick} id={MAX_PRICE_FIRST_SORT} className={`${sort === MAX_PRICE_FIRST_SORT ? 'bg-amber-200/20 text-amber-100 hover:bg-amber-200/60 hover:text-amber-100 active:scale-150' : BUTTON_NEUTRAL_STYLE} w-fit rounded-full py-3 px-4 flex gap-2 transition-all duration-300 cursor-pointer`}>
          <BanknoteArrowUp />
        </button>
      </div>
    </header>
